...
| `1. 列表项`      | 有序列表     | 编号条目                         |
| `> 引用`        | 引用块      | 红色左边框                        |
| `---`         | 分隔线      | MC 中用于分割日/中双栏；Interview 中为横线 |
| `[链接](url)`   | 超链接      | 新标签页打开                       |
| `![描述](图片路径)` | 图片       | 懒加载，圆角阴影                     |
| `[c1]~[c5]`   | 自定义颜色    | 见下方"自定义颜色"                   |

---

## 二、自定义颜色

所有 MD 文件（MC 和 Interview）均支持 10 种自定义颜色：

| 编号  | 色值      | 效果      | 写法                   |
| --- | ------- | ------- | -------------------- |
| c1  | #BB9955 | 金色      | `[c1]金色文字[/c1]`      |
| c2  | #779977 | 绿色      | `[c2]绿色文字[/c2]`      |
| c3  | #335566 | 深蓝      | `[c3]深蓝文字[/c3]`      |
| c4  | #AA4477 | 紫红      | `[c4]紫红文字[/c4]`      |
| c5  | #7799CC | 浅蓝      | `[c5]浅蓝文字[/c5]`      |
...
**2. JS 渲染器**（`live/index.html` 和 `interview/index.html`）

在 `[c1]~[c5]` 颜色处理代码块中追加一行：

```js
html = html.replace(/\[c6\](.+?)\[\/c6\]/g, '<span class="mc-c6">$1</span>');
```
...
generate_all.py
  ...
  ▼
data.js
  │  const xxxData = [{ ..., md_content: "..." }]
  ▼
浏览器 (index.html)
  │  renderMarkdown() → 渲染到页面
  ▼
用户看到的页面
```